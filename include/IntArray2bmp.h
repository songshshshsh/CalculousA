    ...
    for (unsigned row = rows; row; row--)           // bottom-to-top
      ...
      for (unsigned col = 0; col < columns; col++)  // left-to-right
        ...
        #undef c
          ...
          int T_T = intarray[ row - 1 ][ col ];
          while(T_T--)
          {
            red = (red * 3 + 117) % 256;
            green = (green * 5 + 111) % 256;
            blue = (blue * 7 + 103) % 256;
            if(red+green+blue<192 || red+green+blue>768-192 || (red+green-blue >256))// || !(red>blue+green||blue>red+green||green>red+blue))
              if(red+green+blue!=128*3+127+121+113-30)
                T_T++;
          }
